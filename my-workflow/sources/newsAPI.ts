...
export function fetchNewsApiFeed(runtime: Runtime<unknown>, feed: FeedConfig): FeedItem[] {
  const category = feed.category || "news";
  const resolveSeconds = feed.resolveSeconds ?? 24 * 60 * 60;

  if (feed.mock) {
    const value = feed.mockValue ?? "headline";
  ...
  if (!feed.url) {
    throw new Error(`News feed ${feed.id} missing url`);
  }
...
function renderTemplate(template: string | undefined, value: unknown): string {
  const safeTemplate = template || "Will this headline be trending tomorrow: {{value}}?";

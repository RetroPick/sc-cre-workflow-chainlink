/**
 * Swap and sell: credit → buy outcome 0 → swap 0→1 → sell outcome 1.
 * Asserts relayer state and final on-chain state consistent.
 */
...
const SESSION_ID = ("0x" + "00".repeat(31) + "05") as Hex;
...
describe("Anvil swap and sell", { skip: !hasE2EConfig }, () => {
  beforeEach(() => {
    clearAllSessions();
  });

  it(
    "buy outcome 0 → swap 0→1 → sell outcome 1, final state on-chain",
    { skip: !hasE2EConfig },
    async () => {
      ...
      await app.inject({
        ...
        payload: { sessionId: SESSION_ID, userAddress: USER_ADDRESS, amount: 20000 },
      });
      ...
      if (outcomeTokenAddr) {
        ...
        const bal0 = await client.readContract({ address: outcomeTokenAddr as Hex, abi: balanceAbi, functionName: "balanceOf", args: [USER_ADDRESS as Hex, tokenId0] });
        const bal1 = await client.readContract({ address: outcomeTokenAddr as Hex, abi: balanceAbi, functionName: "balanceOf", args: [USER_ADDRESS as Hex, tokenId1] });

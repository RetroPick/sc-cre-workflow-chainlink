/**
 * Challenge window: finalizeCheckpoint reverts before 30 min warp, succeeds after.
 */
...
const SESSION_ID = ("0x" + "00".repeat(31) + "03") as Hex;
...
describe("Anvil challenge window", { skip: !hasE2EConfig }, () => {
  beforeEach(() => {
    clearAllSessions();
  });

  it(
    "finalize reverts before warp, succeeds after warp",
    { skip: !hasE2EConfig },
    async () => {
      ...
      const finalizeBeforeRes = await app.inject({ method: "POST", url: `/cre/finalize/${SESSION_ID}` });
      expect(finalizeBeforeRes.statusCode).not.toBe(200);
      const bodyBefore = JSON.parse(finalizeBeforeRes.payload);
      expect(bodyBefore.error).toBeDefined();
      expect(bodyBefore.error.toLowerCase()).toMatch(/challenge|window|early/);

      await warpPastChallengeWindow(rpcUrl);

      const finalizeAfterRes = await app.inject({ method: "POST", url: `/cre/finalize/${SESSION_ID}` });
      expect(finalizeAfterRes.statusCode).toBe(200);
    }
  );
});
